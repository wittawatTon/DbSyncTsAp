...
WHERE database_id = DB_ID('ReplicationLoadTest');
GO

IF EXISTS (SELECT name FROM sys.databases WHERE name = N'ReplicationLoadTest')
BEGIN
    DROP DATABASE [ReplicationLoadTest];
END
GO

-- Create database if not exists
IF DB_ID('ReplicationLoadTest') IS NULL
BEGIN
    CREATE DATABASE ReplicationLoadTest;
    PRINT 'Database ReplicationLoadTest created.';
END
GO
...
EXEC dbo.CreateReplicationLoadTest @RecordCount = 25000;
GO

select count(*) from ReplicationLoadTest.dbo.SalesDetails sd 
GO

select * from ReplicationLoadTest.dbo.SalesDetails sd where OrderID=250000
GO